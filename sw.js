...
self.addEventListener('install', e => {
  e.waitUntil(Promise.all([
    ...
    caches.open(DATA).then(c => c.add('fonts/UthmanicHafs.woff2')), // шрифтът не се мени — стои при текста
...
self.addEventListener('fetch', e => {
  ...
  // Google шрифтове: CSS идва „непрозрачен“ (no-cors), самите шрифтове — с CORS
  if (url.host === 'fonts.googleapis.com' || url.host === 'fonts.gstatic.com') {
    e.respondWith(cacheFirst(req, SHELL, r => r.ok || r.type === 'opaque'));
    return;
  }
  ...
  // Всичко останало (код, времена за намаз) — мрежа първо; при липса на връзка или над 4 сек — кешът
  e.respondWith((async () => {
    ...
    const slow = new Promise(res => setTimeout(res, 4000)).then(async () => (await cached()) || net);
