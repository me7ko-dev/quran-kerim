...
- `js/intro.js` + `css/intro.css` + маркировка в `index.html` — начален екран „златният мусхаф“ (CSS 3D, без библиотеки).
  ...
  **3D (WebGL):** `tools/intro3d/main.js` (Three.js 0.186) → `npm i && npm run build:intro` → `js/intro3d.js` (минифициран, ~620 KB).
  PBR злато: релеф от карта на височината (Sobel → normal map), собствено „студио“ за отраженията (без RoomEnvironment —
  той изгаря златото фронтално), bloom с праг 1.15, мека контактна сянка. CSS мусхафът се показва веднага и остава,
  ако няма WebGL, ако 3D не е готово преди натискането или ако контекстът се загуби. Пиксели ≤ 2.4 млн.
